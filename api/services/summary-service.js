...
const getDataFromDB = async parms => {
  const data = {};
  data.cats = await categories.findForCity(parms.db, parms.cityId);
  const months = await transactions.findAllTransMonths(parms.db, parms.cityId);
  data.months = await buildMonthsList(months, parms.log);
  data.trans = await transactions.findForMonthlySummary(parms.db, parms.cityId, parms.regular, parms.adhoc);
  data.fctrans = await transactions.findForForecast(parms.db, parms.cityId);
  return data;
};
